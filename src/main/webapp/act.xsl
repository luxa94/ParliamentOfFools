<xsl:stylesheet version="1.0"
            ...
            <xsl:for-each select="a:paragraph">
                ...
                <xsl:for-each select="a:item">
                    ...
                    <xsl:for-each select="a:subItem">
                        ...
                        <p style="text-align: start; padding-left: 20em; padding-right: 20em; margin: 0">-<xsl:value-of select="a:contentItem"/></p>
                        </xsl:for-each>
                    </xsl:for-each>
                </xsl:for-each>
            </xsl:for-each>
    ...
    <xsl:template match="/a:act">
        ...
        <xsl:for-each select="a:preamble/a:contentItem">
            <p style="text-align: start;"><xsl:value-of select="."/></p>
        </xsl:for-each>
